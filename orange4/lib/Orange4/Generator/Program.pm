...
sub generate_func_declaration {
    ...
    if( $statement->{fixed_args_flag} == 1 ){
        ...
        # my $args_count = 1;
        #$declaration_st .= "$statement->{args_num_type}" . " args_num, ... ) {\n";
        $declaration_st .= "signed int" . " args_num, ... ) {\n";
        $declaration_st .= "\tva_list args;\n";

        # for my $arg ( @{$statement->{args_list}} ){
        #     $declaration_st .= "\t$arg->{type} " .  "a$args_count" . ";\n";
        #     $args_count++;
        # }
    ...
    # for my $replace_var (@{$func_vars->[$statement->{st_num}]->{replace_vars}}) {
    #   push  @{$dec_vars}, $replace_var->{$key} if ( defined $replace_var->{$key} && $replace_var->{$key}->{scope} eq "LOCAL" );
    # }
...
sub reset_varset_used {
    ...
    # $self->_func_ret_exp( $func_list, $varset_hash, $unionstructs_hash );
}
