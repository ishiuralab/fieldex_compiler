...
sub new {
    ...
    my $func_list = $args{func_list} || [];
     ...
     bless {
        ...
        replace_vars => $replace_vars, # 最小化で使用ｓる代理変数
        undef_seeds  => [],
        vars         => $vars,
        vars_on_path => {
                          x                => [],
...
sub generate_funcs {
    ...
    while( $self->{func_root_max} > 0 ){
        ...
        for my $func ( @{$self->{func_list}} ){
            # if( $func->{called_flag} != 0 ){
                # push @{$func_st->{called_funcs}}, $func->{st_num};
            # }
            $func->{called_flag} = 0;
        }
        $self->{called} = [];
...
sub _init {
    ...
    my $func_root_max = $self->{func_root_max};
...
sub generate_array {
  ...
  if ($name_type eq 't') { # t 変数には const は使えない
    $array->{modifier} =  random_select( [ 'volatile', '', '', '', '', '', '', '', '', '', '', '', '' ]);
    $array->{scope} = 'GLOBAL';
  }
...
sub generate_statements {
  ...
  sub generate_cases {
      ...
      for my $i( 0 .. $case_num-1 ){
          ...
          # if( $i == $case_num-1 ){
          #     $case_root_num = $root_use_num; # パスの通るところには多めにだすべきでは??

          # }
          # elsif( $root_use_num > 0 ){ # default句まで到達すれば残りの文数全て使う
          #     $case_root_num = int(rand($root_use_num))+1; # 各case内に入る式数を制御できるようにすべきでは??
          # }
          # else{
          #     $case_root_num = 0;
          # }
  ...
  sub generate_func_statement {
      ...
      return +{
          ...
          depth                 => 0,
          struct_args_flag     => $struct_args_flag,
